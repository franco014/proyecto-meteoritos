...
##atributo Onredy
onready var zona_renderizada:TextureRect = $CuadroMiniMapa/ContenedorIconos/ZonaRenderizadoMiniMapa
...
onready var icono_recarga:Sprite = $CuadroMiniMapa/ContenedorIconos/ZonaRenderizadoMiniMapa/EstacionRecarga
	...
	for objeto in objetos_en_ventana:
		if not items_mini_mapa.has(objeto):
			...
			if objeto is BaseEnemiga:
				...
				sprite_icono = icono_recarga.duplicate()
			...
			zona_renderizada.add_child(items_mini_mapa[objeto])
...
func modificar_posicion_iconos() -> void:
	for item in items_mini_mapa:
		...
		var pos_icono: Vector2 = offset_pos * escala_grilla + (zona_renderizada.rect_size * 0.5)
		pos_icono.x = clamp(pos_icono.x, 0, zona_renderizada.rect_size.x)
		pos_icono.y = clamp(pos_icono.y, 0, zona_renderizada.rect_size.y)
		item_icono.position = pos_icono
		
		if zona_renderizada.get_rect().has_point(pos_icono - zona_renderizada.rect_position):
